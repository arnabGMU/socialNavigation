...
class PotentialRewardSAC(BaseRewardFunction):
    """
    Collision reward
    ...
    def __init__(self, config):
        super(PotentialRewardSAC, self).__init__(config)
        self.waypoint_reward_weight = -0.1
    ...
    def get_reward(self, task, env):
        ...
        reward = 0.0
        distance = l2_distance(robot_pos, waypoint)
        if distance > self.waypoint_reach_threshold:
            reward = self.waypoint_reward_weight * distance
        #print("waypoint reward", reward)
        return reward
