...
from occupancy_grid.occupancy_grid import ray_casting, get_lidar, get_gaussian_inflated_pedestrian_map, get_local_map_raycast, get_simple_local_map, plan
...
class Challenge:
        ...
        for ep in range(1, self.val_args.val_episodes_per_scene+1):
            ...
            while not done:
                ...
                if episode_steps == 0:
                    ...
                    episode_start = False
                obs, path_found = self.get_observation(val_env, self.obs_encoder, action=action, first_episode=episode_start, validation=True)
    ...
    def submit(self, agent, args):
        ...
        print(self.args.checkpoint_name)
        while True:
            #print(f"episode: {total_num_episodes+1}")
            env.initialize_episode()
            episode_reward = 0
            episode_steps = 0
            done = False
            ...
            while not done:
                ...
                # if env.replan and env.ghost_node is not None:
                #     ped_distances = []
                #     for i, ped in enumerate(env.pedestrians):
                #         ped_pos = env.world_to_map(np.array(ped[0]))
                #         ped_distances.append(dist(ped[0], env.waypoints[0]))
                #     print(ped_distances)
                #     print(env.ghost_node)
                #     env.visualize_map()
                # continue
